...
// ============================================================
// ELITE HUMAN-LIKE PERSONA ENGINE v2
// Fully fixed and optimized
// 20 Avatar Sources, Telegram-style initials
// Emoji + Titles + Crypto Aliases
// No duplicates
// Gender + Country aware
// Slang engine expanded
// Conversation memory + Reply chains
// Trending simulation + Emotions
...
function generateSyntheticPersona(){
    ...
    let name; do{name=buildUniqueName(gender);}while(UsedNames.has(name));
    UsedNames.add(name);
    const persona={
        name,
...
function getPersona({type="synthetic"}={}){return type==="admin"?Admin:getRandomPersona();}
function isDuplicate(postId,text){if(!PostComments[postId])PostComments[postId]=new Set();if(PostComments[postId].has(text)) return true;PostComments[postId].add(text);return false;}
function generateTimestamp(){const now=new Date();const past=new Date(now);const mode=rand(3);if(mode===0) past.setMinutes(now.getMinutes()-rand(60));else if(mode===1) past.setHours(now.getHours()-rand(24));else past.setDate(now.getDate()-rand(30));return past;}
